...
import { fetchStudentData, fetchStudyPlan } from "../state/actions";
import { AppDispatch, RootState } from "../state/store";
import "./visualization.css";

const VisualizationPage: React.FC = () => {
  const curriculum = useSelector((state: RootState) => state.curriculum.years);
  const studentInfo = useSelector((state: RootState) => state.curriculum.studentInfo);
  const student = studentInfo?.[0];
  const loading = useSelector((state: RootState) => state.curriculum.loading);
  const error = useSelector((state: RootState) => state.curriculum.error);
  const dispatch = useDispatch<AppDispatch>();
  const svgRef = useRef<SVGSVGElement>(null);

  useEffect(() => {
    const studentId = 1001; // Mocked stdId
    ...
    dispatch(fetchStudentData(studentId))
      ...
      .then(() => {
        dispatch(fetchStudyPlan(studentId))
          .unwrap()
          .then((studyPlan) => {
            console.log("Raw Study Plan:", studyPlan);
          })
          .catch((error) => {
                  ...
                  {year.semesters.map((semester: any, semesterIndex: number) => (
                    ...
                    >
                      <FormControlLabel
                        ...
                        label={`Drop Semester ${semester.semester}`}
                      />
                      {!semester.dropped &&
                          ...
                          >
                            ...
                            >
                              {subject.grade && (
                                ...
                                >
                                  {subject.grade === "Undefinded" ? "" : subject.grade}
                                </Typography>
                              )}
                              <Typography variant="body2" fontWeight="bold">
                                {subject.code}
